...
| 🃏 **Ronda** | 2, 3 or 4 | [`/game/ronda/`](https://aminou973.github.io/cybergrill/game/ronda/) |
...
pair taking both points when two are called — **and you may keep quiet**. A
weak pair calls the point straight down on somebody holding a better one, so
sitting on it is often right. Get caught playing that pair and everybody else
takes a point.

In play: **darba** 1 for taking a card straight back off the player before you,
then **b'khamsa** 5 and **b'ashra** 10 as the third and fourth of that rank
follow. The escalation runs **round the table, not down one side** — each step
pays whoever laid the card, so in a four-hander the points alternate between
the teams. **Missa** 1 for clearing the table, and a darba that clears it pays
both.

**Qa'a** — the dealer is dealt last, so the final card of the deck is theirs. A
king down there is +5 to the dealer's side; an ace is +5 against them.

At the end, **bawesh** sweeps to whoever captured last, then a point for every
card over the neutral share — twenty with two or four players, thirteen with
three. First to 41.
...
The table explains itself as it goes. Captured cards travel to the winner's
pile rather than vanishing, every move gets a line underneath saying what it
did and why, and a **pace** chip runs from *teaching* down to *brisk* so you
can slow it right down for somebody learning and speed it back up once they
are not.
