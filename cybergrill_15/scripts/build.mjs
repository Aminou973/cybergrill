...
function buildDashboard() {
  ...
  const assets = ['i18n.js', 'i18n-prose.js', 'card.js', 'scorebook.js']
...
function buildTable(t) {
  ...
  /* the shared plumbing: French and Arabic, and the tray results go into on
     their way to the scoreboard */
  const lre = /\/\* SHARED:BEGIN \*\/[\s\S]*?\/\* SHARED:END \*\//;
  if (lre.test(html)) {
    const langs = ['i18n-game.js', 'lang.js', 'scorebook.js', 'voice.js']
      .map(x => join(ROOT, 'shared', x))
      .filter(x => existsSync(x))
      .map(x => readFileSync(x, 'utf8'))
      .join('\n');
    if (!langs) console.log('· shared/lang.js is missing — tables will be English only');
    else html = html.replace(lre, langs);
  }
